...
int	 nOrdinamento = 0;	
	...
	while(!feof(f))
	{
		...
		if( strncmp(buffLine, "Nome", 4) == 0)
		{
			...
			} while(strncmp(buffLine, "Colonna", 7) != 0);
			...
			*/	
			...
			short esit = false;
			esit  = CheckLabs(nOrdinamento, 0);
			esit += CheckSpec(nOrdinamento, 0);
			esit += CheckGest(nOrdinamento);
			esit += CheckBase(nOrdinamento, 0); 
			esit += CheckSisF(nOrdinamento, 0); 
			esit += CheckBiom(nOrdinamento);
			esit += CheckInge(nOrdinamento);
			esit += CheckMatChf(nOrdinamento,0);
